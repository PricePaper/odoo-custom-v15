...
class AddDiscount(models.TransientModel):
    ...
    def create_discount(self):
        ...
        discount_limit = self.env['ir.config_parameter'].sudo().get_param(
        ...
        if self.env.context.get('type') == 'out_invoice' and discount_amount > discount_limit:
            raise UserError(_('Invoices can not be discounted more than $ %.2f. \nCreate a credit memo instead.' % discount_limit))

        if float_compare(discount_amount, move.amount_residual, precision_digits=2) > 0:
            raise UserError(_('Cannot add discount more than residual $ %.2f' % move.amount_residual))
        ...
        discount_move = self.env['account.move'].create({
            ...
            'line_ids': [(0, 0, {
                ...
                'credit': 0.0 if self.env.context.get('type') == 'in_invoice' else discount_amount,
                'debit': discount_amount if self.env.context.get('type') == 'in_invoice' else 0.0,
                'journal_id': move.journal_id.id,
                'name': 'Discount',
                ...
                'company_currency_id': company_currency.id,
                'credit': discount_amount if self.env.context.get('type') == 'in_invoice' else 0.0,
                'debit': 0.0 if self.env.context.get('type') == 'in_invoice' else discount_amount,
                'journal_id': move.journal_id.id,
                'name': 'Discount',
                'partner_id': move.partner_id.id
            })]
