...
class PaymentAcquirer(models.Model):
    ...
    provider = fields.Selection(
        selection_add=[('cod', "Cod")], default='cod',
        ondelete={'cod': 'set default'})
    qr_code = fields.Boolean(
        string="Enable QR Codes", help="Enable the use of QR-codes when paying by Cod.")
    ...
    def create(self, values_list):
        ...
        acquirers._cod_ensure_pending_msg_is_set()
        return acquirers
    ...
    def write(self, values):
        ...
        self._cod_ensure_pending_msg_is_set()
        return res

    def _cod_ensure_pending_msg_is_set(self):
        for acquirer in self.filtered(lambda a: a.provider == 'cod' and not a.pending_msg):
