    ...
    def onchange_load_data(self):
        ...
        for data in self:
            ...
            for row in reader:
                ...
                if count == 0:
                    count += 1
                    if self._context.get('is_competitor', False) == 'competitor' and (
                            row[0].lower().strip() not in ('product_ref', 'product ref') or row[
                        1].lower().strip() not in ('price')):
                        msg = 'Incompatiable csv file!\nThe column headers should be \Product ref,Price\' and should also follow the same order.'
                    else:
                        if self._context.get('is_competitor', False) != 'competitor' and (
                                row[0].lower().strip() not in ('product_ref', 'product ref') or row[
                            1].lower().strip() not in ('customer_code', 'customer code') or row[
                                    2].lower().strip() not in ('price')):
                    ...
                    if not product:
                        return {'warning': {'title': 'Error!',
                                            'message': 'No Product found for Internal Reference %s' % row[0].strip()}}
                        break
                    ...
                    if self._context.get('is_competitor', False) != 'competitor':
                        ...
                        if not customer:
                            return {'warning': {'title': 'Error!',
                                                'message': 'No Customer found for Customer Code %s' % row[1].strip()}}
                            break
