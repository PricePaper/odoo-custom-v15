...
class AccountRegisterPayment(models.TransientModel):
    ...
    def _compute_payment_amount(self, invoices=None, currency=None):
        if not self.id or (self.id and not self.multi):
        ...
        if not self.writeoff_account_id and discount_amount:
            ...
            if invoice:
                if invoice.type in ('out_invoice', 'in_refund'):
                    self.writeoff_account_id = invoice.company_id.cash_discount_id
                else:
                    self.writeoff_account_id = invoice.company_id.vendor_discount_id
    ...
    def default_get(self, fields):
        ...
        for invoice in invoices:
            ...
            if invoice.type in ('out_invoice', 'in_refund', 'out_refund'):
                wo_account = invoice.company_id.cash_discount_id
                reference = invoice.origin
            else:
                wo_account = invoice.company_id.vendor_discount_id
                reference = invoice.reference
            discount_amount = 0
            discount = 0
            if invoice.payment_term_id.is_discount:
                discount_days = invoice.payment_term_id.discount_days
                if discount_days >= 0:
                    invoice_date = datetime.strptime(invoice.date_invoice, '%Y-%m-%d')
                    now = datetime.strptime(time.strftime("%Y-%m-%d"), '%Y-%m-%d')
                    difference = (now - invoice_date).days
                    if difference <= discount_days and invoice.payment_term_id.discount:
                        flag = True
                        discount = invoice.payment_term_id.discount or 1
                        discount_percentage = discount / 100
                        discount_amount = invoice.residual * discount_percentage
            discount_amount = round(discount_amount, 2)
            amount = invoice.residual_signed - discount_amount
#            amount = invoice.residual - discount_amount
            amount_total += amount
...
class AccountPayment(models.Model):
    ...
    def _create_payment_entry(self, amount):
        ...
        if self.has_payment_lines and self.payment_lines.filtered(lambda record: record.is_full_reconcile == False):
            ...
            debit, credit, amount_currency, currency_id = aml_obj.with_context(
                date=self.payment_date).compute_amount_fields(amount, self.currency_id, self.company_id.currency_id,
                                                              invoice_currency)
            x = self._get_move_vals()

            move = self.env['account.move'].create(x)
            ...
            if self.payment_difference_handling == 'reconcile' and self.payment_difference:
                ...
                debit_wo, credit_wo, amount_currency_wo, currency_id = aml_obj.with_context(
                    date=self.payment_date).compute_amount_fields(payment_difference, self.currency_id,
            ...
            reconciled_lines = self.payment_lines.filtered(lambda record: record.is_full_reconcile == True)
            ...
            if self.payment_lines.filtered(lambda record: record.is_full_reconcile == False):
                ...
                debit_r, credit_r, amount_currency_r, currency_id_r = aml_obj.with_context(
                    date=self.payment_date).compute_amount_fields(amount_r, self.currency_id,
                                                                  self.company_id.currency_id, invoice_currency)
                ...
                for line in self.payment_lines.filtered(lambda record: record.is_full_reconcile is False):
                    ...
                    debit_t, credit_t, amount_currency_t, currency_id_t = aml_obj.with_context(
                        date=self.payment_date).compute_amount_fields(amount_t, self.currency_id,
                                                                      self.company_id.currency_id, invoice_currency)
