...
class GenerateDiscountCheck(models.TransientModel):
    ...
    def get_invoices(self):
        ...
        if self.start_date and self.end_date:
            vendor_bills1 = self.env['account.move']
            vendor_bills = self.env['account.move'].search([('state','=','posted'), ('move_type','=','in_invoice'), ('payment_state', 'in', ('not_paid', 'partial')), ('discount_due_date', '>', self.start_date), ('discount_due_date', '<', self.end_date)])

            vendor_bills1 = self.env['account.move'].search([('state','=','posted'), ('move_type','=','in_invoice'), ('payment_state', 'in', ('not_paid', 'partial')), ('invoice_date_due', '>', self.start_date), ('invoice_date_due', '<', self.end_date), ('discount_due_date', '=', False)])
            ...
            for bill in vendor_bills:
                if bill.discount_due_date and bill.discount_due_date >= fields.Date.today():
                    discount_total_amount = bill.amount_residual_signed * (100 - bill.payment_term_id.discount_per) / 100
                else:
                    discount_total_amount = 0.00
                val = {
                    'total_amount': bill.amount_residual_signed,
            ...
            # self.wizard_invoice_ids = [0, 0, lines]
            # return {'self' : vals}


    # TODO register payment with discount in
...
class VendorBillLines(models.TransientModel):
    ...
    invoice_id_pseudo = fields.Many2one('account.move', string='Invoice')
